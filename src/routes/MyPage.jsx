...
export default function MyPage() {
    ...
    useEffect(() => {
        ...
        // user.tel이 있는 경우에만 값을 설정하고 useEffect 종료
        if (userData.user.tel) {
            setTel(userData.user.tel);
        }
    ...
    const handleSubStateFalse = async () => {
        const res = await apiPostUserSub(false, userid);
        sessionStorage.setItem('userData', JSON.stringify(res));
        alert('구독해제 되었습니다.');
    };
    const handleSubStateTrue = async () => {
        const res = await apiPostUserSub(true, userid);
        sessionStorage.setItem('userData', JSON.stringify(res));
        alert('구독신청 되었습니다.');
    };
    return (
                                    ...
                                    {Object.keys(allergies).map((allergy) => (
                                        <div
                                            ...
                                            className={`border border-blue-500 cursor-pointer text-blue-500 py-2 px-4 rounded-full m-2 hover:bg-blue-500 hover:text-white transition duration-300 ${
                                                allergies[allergy] ? 'bg-blue-500 text-white border-white' : ''
                                            }`}
                            ...
                            >
                                <div className="w-full max-w-[1000px]">
                                    ...
                                    <div className="w-full xl:h-[55%] mb-10 mt-1 flex flex-col xl:justify-between items-center bg-white border rounded-xl p-4">
                                        ...
                                        {/* <button className="w-1/2 mt-16 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-6 rounded">
                                            선택하기
                                        </button> */}
                                    </div>
                                    <div className="  w-full sm:h-[30%] flex flex-col items-center bg-white border rounded-xl p-4 relative">
                                        {/* 학교이름 */}
                                        <h4>학교 정보 수정</h4>
