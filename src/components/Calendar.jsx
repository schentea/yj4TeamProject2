...
const MealCalendar = ({ closeModal }) => {
    ...
    const formatMealData = (data) => {
        ...
        const formattedMenu = menuItems.map((item) => {
            ...
            const containsAllergy = ingredientNumbers.some((ingredientNumber) =>
                allergyNumber.includes(ingredientNumber)
            ); // 알레르기 번호 포함 여부 확인
            ...
            <div className="w-full relative max-w-3xl overflow-y-scroll h-[95%] flex flex-col bg-white p-8 rounded-lg shadow-xl">
                ...
                <div className="max-w-[90%] mx-auto mt-8">
                    <div className="flex justify-center mb-4">
                        ...
                        <button
                            className={`mr-4 ${activeTab === 'allergy' ? 'text-blue-600' : ''}`}
                            onClick={() => handleTabClick('allergy')}
                        >
                            알레르기 정보
                        </button>
                        <button
                            className={`mr-4 ${activeTab === 'origin' ? 'text-blue-600' : ''}`}
